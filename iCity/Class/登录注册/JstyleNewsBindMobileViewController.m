//
//  JstyleNewsBindMobileViewController.m
//  JstyleNews
//
//  Created by JingHongMuYun on 2018/1/16.
//  Copyright © 2018年 JstyleNews. All rights reserved.
//

#import "JstyleNewsBindMobileViewController.h"

@interface JstyleNewsBindMobileViewController ()
...
@property (nonatomic, strong) JstyleNewsBaseView *holdView;
...
@implementation JstyleNewsBindMobileViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"绑定手机号";
    self.view.backgroundColor = kWhiteColor;
    [self addLeftBarButtonWithImage:(ISNightMode?JSImage(@"返回白色"):JSImage(@"图文返回黑")) action:@selector(leftBarButtonAction)];
    [self setUpViews];
}
...
- (void)bindMobileDataSource
{
    ...
    NSDictionary *phoneCode = @{@"phone":_mobileField.text,
                                @"code":_codeField.text};
    NSMutableDictionary *parameters = [NSMutableDictionary dictionary];
    [parameters setValuesForKeysWithDictionary:phoneCode];
    [parameters setValuesForKeysWithDictionary:self.userInfo];
    
    [[JstyleNewsNetworkManager shareManager] POSTURL:BIND_MOBILE_URL parameters:parameters success:^(id responseObject) {
        if ([responseObject[@"code"] integerValue] == 1) {
...
- (void)addSuccessView
{
    ...
    [_codeField resignFirstResponder];
    _isBindSuccess = YES;
    _holdView = [[JstyleNewsBaseView alloc] init];
    ...
    _successImageView.image = JSImage(@"绑定成功");
    [_holdView addSubview:_successImageView];
    _successImageView.sd_layout
    .centerXEqualToView(_holdView)
    .centerYEqualToView(_holdView).offset(-18)
    .widthIs(50)
    .heightIs(50);
    
    _successLabel = [[UILabel alloc] init];
    _successLabel.font = JSFont(18);
    _successLabel.textColor = ISNightMode?kDarkNineColor:kDarkFiveColor;
    _successLabel.textAlignment = NSTextAlignmentCenter;
    _successLabel.text = @"手机号绑定成功!";
    [_holdView addSubview:_successLabel];
    _successLabel.sd_layout
    .leftSpaceToView(_holdView, 15)
    .rightSpaceToView(_holdView, 15)
    .topSpaceToView(_successImageView, 35)
    .heightIs(18);
