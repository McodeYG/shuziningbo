...
@property (nonatomic, copy) void(^lastOneItemBlock)(BOOL isLastOneItem);
...
- (NSMutableArray *)dataList {
    ...
    if (_dataList.count == 1) {
        isLastOneItem = YES;
        [[NSNotificationCenter defaultCenter] postNotificationName:@"HXPhotoViewDataListCountChangeNotification" object:nil userInfo:@{@"isLastOneItem":@"YES"}];
    } else {
        isLastOneItem = NO;
        [[NSNotificationCenter defaultCenter] postNotificationName:@"HXPhotoViewDataListCountChangeNotification" object:nil userInfo:@{@"isLastOneItem":@"NO"}];
    }
    if (self.lastOneItemBlock) {
        self.lastOneItemBlock(isLastOneItem);
    }
...
- (void)setupNewFrame {
    ...
    NSInteger dataCount = self.dataList.count > 9 ? self.dataList.count - 1 : self.dataList.count;
...
- (void)layoutSubviews {
    ...
    ///还剩一个时改变collectionView的Frame,使其居中
    __weak typeof(self)weakSelf = self;
    self.lastOneItemBlock = ^(BOOL isLastOneItem){
        if (isLastOneItem) {
            weakSelf.collectionView.frame = CGRectMake(cWidth/3.0, 0, (cWidth-6)/3.0, (cWidth-6)/3.0);
        } else {
            weakSelf.collectionView.frame = CGRectMake(0, 0, cWidth, cHeight);
        }
    };
}
- (void)dealloc {
