...
static CGFloat const lineWGap = 10*xGap;//tg:底部线的长度
...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        titleView.text = (title == nil ? @"" : title);
...
- (void)setupScrollViewAndExtraBtn {
    ...
    //        scrollW = maxX;
    //    }
    if (self.isChannelTags) {
        self.scrollView.frame = CGRectMake(5.0, 0.0, scrollW - 10, self.zj_height);
    }else{
        self.scrollView.frame = CGRectMake(0.0, 0.0, scrollW, self.zj_height);
    }

    if (self.extraBtn) {
        self.extraBtn.frame = CGRectMake(scrollW - 5, extraBtnY, extraBtnW, self.zj_height - 2*extraBtnY);
    }
}
...
- (void)setUpTitleViewsPosition {
    ...
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        if (self.segmentStyle.isChangeScrollViewFrame) {
            titleW = (self.scrollView.bounds.size.width - 80 *kScreenWidth/375.0) / self.titles.count;
        }else{
    ...
    if (currentTitleView) {
        ...
        currentTitleView.font = self.segmentStyle.titleFont;
        if (self.segmentStyle.isShowImage) {
...
- (void)setupScrollLineAndCover {
    ...
    CGFloat coverY = (self.bounds.size.height - coverH) * 0.45;
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            ...
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                coverW = [self.titleWidths[_currentIndex] floatValue] + lineWGap;
    ...
    if (self.coverLayer) {//显示遮盖
        
        if (self.segmentStyle.layerWith>0) {//自己加的
            
            coverX = 10;
            coverW = self.segmentStyle.layerWith;
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        if (self.clickTextNormalColorBlock) {
            self.clickTextNormalColorBlock(oldTitleView);
        } else {
            oldTitleView.textColor = weakSelf.segmentStyle.normalTitleColor;
        }
        oldTitleView.font = weakSelf.segmentStyle.titleFont;
        if (self.clickTextSelectColorBlock) {
            self.clickTextSelectColorBlock(currentTitleView);
        } else {
            currentTitleView.textColor = weakSelf.segmentStyle.selectedTitleColor;
        }
        if (weakSelf.segmentStyle.selectedTitleFont) {
            currentTitleView.font = weakSelf.segmentStyle.selectedTitleFont;
        }
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                ...
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    CGFloat scrollLineW = [self.titleWidths[_currentIndex] floatValue] + lineWGap;
        ...
        if (weakSelf.coverLayer) {
            ...
            if (weakSelf.segmentStyle.layerWith>0) {
                //自己加的
                weakSelf.coverLayer.zj_width = weakSelf.segmentStyle.layerWith;
                
                weakSelf.coverLayer.zj_x = 10+_currentIndex*(weakSelf.segmentStyle.layerWith+10);
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            ...
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                CGFloat oldScrollLineW = [self.titleWidths[oldIndex] floatValue] + lineWGap;
                CGFloat currentScrollLineW = [self.titleWidths[currentIndex] floatValue] + lineWGap;
    ...
    if (self.coverLayer) {
        ...
        if (self.segmentStyle.layerWith>0) {
            //我的
            self.coverLayer.zj_x = 10+(oldIndex * (self.segmentStyle.layerWith+10) )+ xDistance * progress;
            self.coverLayer.zj_width = self.segmentStyle.layerWith;
    ...
    if (self.segmentStyle.isGradualChangeTitleColor) {
        ...
        if (self.textColorBlock) {
            self.textColorBlock(oldTitleView, currentTitleView);
        }
    }
...
- (void)adjustTitleOffSetToCurrentIndex:(NSInteger)currentIndex {
    ...
    for (ZJTitleView *titleView in _titleViews) {
        if (index != currentIndex) {
            ...
            titleView.textColor = self.segmentStyle.normalTitleColor;
//            if (self.clickTextNormalColorBlock) {
//                self.clickTextNormalColorBlock(titleView);
//            }
            titleView.currentTransformSx = 1.0;
            titleView.selected = NO;
        }else {
            if (self.segmentStyle.selectedTitleFont) {
                titleView.font = self.segmentStyle.selectedTitleFont;
            }
            titleView.textColor = self.segmentStyle.selectedTitleColor;
//            if (self.clickTextSelectColorBlock) {
//                self.clickTextSelectColorBlock(titleView);
//            }
            if (self.segmentStyle.isScaleTitle) {
...
- (UIButton *)extraBtn {
    ...
    if (!_extraBtn) {
        ...
        btn.contentEdgeInsets = UIEdgeInsetsMake(- 2, 5, 0, 0);
...
//        btn.layer.shadowOpacity = 1;
        ...
        _extraBtn.lee_theme
        .LeeConfigButtonImage(ThemeChannelBarExtraBtnImage, UIControlStateNormal)
        .LeeConfigButtonImage(ThemeChannelBarExtraBtnImage, UIControlStateHighlighted);
    }
    return _extraBtn;
}
