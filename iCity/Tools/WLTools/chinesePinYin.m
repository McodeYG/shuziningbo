...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr
{
    ...
    {
        NSString *pinyin = [((chinesePinYin*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr
{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((chinesePinYin*)object).pinYin substringToIndex:1];
        NSString *string = ((chinesePinYin*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    {
        chinesePinYin *chineseString = [[chinesePinYin alloc]init];
...
//        chineseString.string = [chinesePinYin RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)SortArray:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++){
        [result addObject:((chinesePinYin*)[tempArray objectAtIndex:i]).string];
//        NSLog(@"SortArray----->%@",((chinesePinYin*)[tempArray objectAtIndex:i]).string);
    }
    return result;
}
