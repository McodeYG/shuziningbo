...
@interface VRPlayerPlayButton ()<CAAnimationDelegate> {
 ...
 */
- (void)buildUI {
    _buttonState = VRPlayerPlayButtonStatePause;
...
- (void)addTriangleLayer {
    ...
    [path moveToPoint:CGPointMake(a*0.15,a*0.15)];
    [path addLineToPoint:CGPointMake(a*0.15,0)];
    [path addLineToPoint:CGPointMake(a,a*0.5)];
    [path addLineToPoint:CGPointMake(a*0.15,a)];
    [path addLineToPoint:CGPointMake(a*0.15,a*0.15)];
...
- (void)addLeftLineLayer {
    ...
    UIBezierPath *path = [UIBezierPath bezierPath];
    [path moveToPoint:CGPointMake(a*0.15,0)];
    [path addLineToPoint:CGPointMake(a*0.15,a)];
...
- (void)addRightLineLayer {
    ...
    UIBezierPath *path = [UIBezierPath bezierPath];
    [path moveToPoint:CGPointMake(a*0.85,a)];
    [path addLineToPoint:CGPointMake(a*0.85,0)];
...
- (void)addCircleLayer {
    ...
    CGFloat a = self.bounds.size.width;
    UIBezierPath *path = [UIBezierPath bezierPath];
    [path moveToPoint:CGPointMake(a*0.85,a*0.85)];
    [path addArcWithCenter:CGPointMake(a*0.5, a*0.85) radius:0.3*a startAngle:0 endAngle:M_PI clockwise:true];
...
- (void)actionPositiveAnimation {
    ...
    //开始逆向画弧动画
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,  animationDuration*0.155 * NSEC_PER_SEC), dispatch_get_main_queue(), ^(void){
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,  animationDuration*0.5 * NSEC_PER_SEC), dispatch_get_main_queue(), ^(void){
        ...
        _triangleLayer.fillColor = LineColor.CGColor;
    });
}
...
-(void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    ...
    bool isTriangle = [name isEqualToString:TriangleAnimation];
    bool isRightLine = [name isEqualToString:RightLineAnimation];
    if (_buttonState == VRPlayerPlayButtonStatePlay && isRightLine) {
...
//线条宽度，根据按钮的宽度按比例设置
- (CGFloat)lineWidth {
    return self.bounds.size.width * 0.4;
}
...
- (void)linePositiveAnimation {
    ...
    [leftPath1 moveToPoint:CGPointMake(0.15*a,0.4*a)];
    [leftPath1 addLineToPoint:CGPointMake(0.15*a,a)];
    ...
    [rightPath1 moveToPoint:CGPointMake(0.85*a, 0.85*a)];
    [rightPath1 addLineToPoint:CGPointMake(0.85*a,-0.15*a)];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,  positionDuration/2 * NSEC_PER_SEC), dispatch_get_main_queue(), ^(void){
        ...
        [leftPath2 moveToPoint:CGPointMake(a*0.15,a*0.15)];
        [leftPath2 addLineToPoint:CGPointMake(a*0.15,a*0.85)];
        _leftLineLayer.path = leftPath2.CGPath;
        [_leftLineLayer addAnimation:[self pathAnimationWithDuration:positionDuration/2] forKey:nil];
        
        //右侧竖线缩放动画
        UIBezierPath *rightPath2 = [UIBezierPath bezierPath];
        [rightPath2 moveToPoint:CGPointMake(a*0.85,a*0.85)];
        [rightPath2 addLineToPoint:CGPointMake(a*0.85,a*0.15)];
...
- (void)lineInverseAnimation {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,  positionDuration/2 * NSEC_PER_SEC), dispatch_get_main_queue(), ^(void){
        //左侧竖线缩放动画
        UIBezierPath *leftPath2 = [UIBezierPath bezierPath];
        [leftPath2 moveToPoint:CGPointMake(a*0.15,0)];
        [leftPath2 addLineToPoint:CGPointMake(a*0.15,a)];
        _leftLineLayer.path = leftPath2.CGPath;
        [_leftLineLayer addAnimation:[self pathAnimationWithDuration:positionDuration/2] forKey:nil];
        
        //右侧竖线缩放动画
        UIBezierPath *rightPath2 = [UIBezierPath bezierPath];
        [rightPath2 moveToPoint:CGPointMake(a*0.85,a)];
        [rightPath2 addLineToPoint:CGPointMake(a*0.85,0)];
        _rightLineLayer.path = rightPath2.CGPath;
        [_rightLineLayer addAnimation:[self pathAnimationWithDuration:positionDuration/2] forKey:nil];
    });
}
...
#pragma mark -
#pragma mark Setter
- (void)setButtonState:(VRPlayerPlayButtonState)buttonState {
    //如果正在执行动画则不再执行下面操作
    if (_isAnimating == true) {return;}
    _buttonState = buttonState;
    
    if (buttonState == VRPlayerPlayButtonStatePlay) {//暂停-》播放
        _isAnimating = true;
        //竖线正向动画
    ...
    } else if (buttonState == VRPlayerPlayButtonStatePause) {//播放-》暂停
        _isAnimating = true;
        //先执行画弧、画三角动画
